...
void main() {
    ...
    vec3 noise1 = vec3(vUv.x*0.0005+u_time*0.05, vUv.y*0.0005, 1.0);
    ...
    {
      float fact = exp(i);
      curvePos.z += (1.0/(fact*0.01)) *snoise(fact*noise1, temp) *(sin(u_time) +1.1);
      grad1 += temp*(sin(u_time) +1.1); //multiply temp every time with the same as the snoise function
    }
     ...
     //set position
     gl_Position = projectionMatrix*viewMatrix*vec4(curvePos,1.0);
 ...
 /* vec3 grad; //gradient to be used in new normal calculation
  ...
  {
    float f = exp(i);
    newpos.z += 0.5*snoise(f*0.005*position+vec3(0.2*u_time), temp);
    temp2 += temp;
  }
